...
/*End function get Object Length*/

var nse_arvind_stock_info = Drupal.settings.arvind_common.nse_arvind_stock_info;
var myDataStock = [];
for (i = 1; i < Object.size(nse_arvind_stock_info); i++) {

    var year = nse_arvind_stock_info[i]['y'];
    var month = nse_arvind_stock_info[i]['m'] -1 ;
    var date = nse_arvind_stock_info[i]['d'];
    var price = parseFloat(nse_arvind_stock_info[i]['p']);

    var time = Date.UTC(year, month, date);

    myDataStock.push([time, price]);
...
jQuery(window).load(function(){
    jQuery('#cust_quarter').change(function(){
        var quarter = jQuery(this).val();
        var year = jQuery('.yearsliderwrap.annualreyear .slick-current').text();
        // console.log('quarter: '+quarter);
        // console.log('year: '+year);
        quarter = quarter.replace(/ /g, '');
        quarter = quarter.toLowerCase();
        var classname = quarter.concat('_'+year);
        // alert(classname);
        jQuery('.annualconten .pdfFileList.box4width li').hide();
        jQuery('.'+classname).show();

    });

    jQuery('.annualreyear.yearslide').click(function(){
        var quarter = jQuery("#cust_quarter").val();
    ...
    //Start Quarterly Report JS

    jQuery('#cust_quarterQ').change(function(){
        var quarter = jQuery(this).val();
        var year = jQuery('.yearsliderwrap.quaterlyYear .slick-current').text();
        quarter = quarter.replace(/ /g, '');
        quarter = quarter.toLowerCase();
        quarter = 'Q_'+quarter;
        var classname = quarter.concat('_'+year);
        jQuery('.quaterlyConten .pdfFileList.box4width li').hide();
        jQuery('.'+classname).show();

    });

    jQuery('.quaterlyYear .yearslide').click(function(){
        quaterlyYear_change();
    });

    jQuery('.quaterlyYear').on('afterChange', function(event, slick, currentSlide, nextSlide){
        console.log('xxxx: '+jQuery('.yearsliderwrap.quaterlyYear .slick-current').text());
        quaterlyYear_change();
    });
    
});

function quaterlyYear_change(){
    var quarter = jQuery('#cust_quarterQ').find("option:selected").val();
    var year    = jQuery('.yearsliderwrap.quaterlyYear .slick-current').text();
    quarter     = quarter.replace(/ /g, '');
    quarter     = quarter.toLowerCase();
    quarter     = 'Q_'+quarter;
...
$('.annualconten').slick({
    ...
    fade: true,
    infinite: false,
    asNavFor: '.annualreyear'
});
$('.annualreyear').slick({
    slidesToShow: 5,
    slidesToScroll: 1,
    asNavFor: '.annualconten',
    dots: false,
    infinite: false,
    focusOnSelect: true,
    responsive: [
...
$('.quaterlyConten').slick({
    slidesToShow: 1,
    slidesToScroll: 1,
    arrows: false,
    infinite: false,
    fade: true,
    asNavFor: '.quaterlyYear'
});
$('.quaterlyYear').slick({
    slidesToShow: 5,
    slidesToScroll: 1,
    asNavFor: '.quaterlyConten',
    dots: false,
      infinite: false,
    focusOnSelect: true,
   responsive: [
...
$('.quaterlyContena').slick({
    slidesToShow: 1,
    slidesToScroll: 1,
    arrows: false,
    infinite: false,
    fade: true,
    asNavFor: '.quaterlyYeara'
});
$('.quaterlyYeara').slick({
    slidesToShow: 5,
    slidesToScroll: 1,
    asNavFor: '.quaterlyContena',
    dots: false,
      infinite: false,
    focusOnSelect: true,
   responsive: [
...
$('.peryearslide').slick({
  ...
  fade: true,
  infinite: false,
  asNavFor: '.yearsliderwrap'
});
$('.yearsliderwrap').slick({
  slidesToShow: 5,
  slidesToScroll: 1,
  asNavFor: '.peryearslide',
  dots: false,
  infinite: false,
  focusOnSelect: true,
    responsive: [
